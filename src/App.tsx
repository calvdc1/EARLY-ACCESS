...
export default function App() {
  const validViews = ['home', 'explorer', 'about', 'dashboard', 'campus-detail', 'messenger', 'newsfeed', 'profile', 'confession', 'feedbacks', 'lostfound', 'scheduler'] as const;
  ...
  const [view, setView] = useState<'home' | 'explorer' | 'about' | 'dashboard' | 'campus-detail' | 'messenger' | 'newsfeed' | 'profile' | 'confession' | 'feedbacks' | 'lostfound' | 'scheduler'>(() => {
  ...
  const [newsfeedPosts, setNewsfeedPosts] = useState<{ id: number; user_id: number; user_name: string; user_avatar?: string; campus: string; content: string; image_url?: string; likes: number; comments: number; timestamp: string }[]>([]);
  const [newsfeedText, setNewsfeedText] = useState('');
  const [newsfeedImagePreview, setNewsfeedImagePreview] = useState<string | null>(null);
  ...
  const renderDashboard = () => {
    ...
    const quickActions = [
      ...
      { name: 'Explore', icon: <Globe size={16} />, action: () => setView('explorer') },
      { name: 'Feedbacks', icon: <Info size={16} />, action: () => setView('feedbacks') }
    ];

    return (
    <div className="h-screen w-full bg-[radial-gradient(circle_at_top,rgba(61,43,22,0.9),#090704_55%)] flex flex-col">
      {/* Top Navigation Bar */}
      <nav className="border-b border-white/10 bg-black/40 backdrop-blur-sm sticky top-0 z-40">
        <div className="px-4 md:px-8 py-3">
          <div className="max-w-7xl mx-auto">
            {/* Header Row */}
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                <h2 className="text-lg md:text-xl font-bold text-white">Welcome, {user?.name || 'MSUan'}!</h2>
              </div>
            ...
            <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
              {quickActions.map(item => (
                <button
                  key={item.name}
                  onClick={() => item.action?.()}
                  className="flex items-center gap-2 px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-xs font-medium hover:bg-amber-500 hover:text-black transition-colors whitespace-nowrap relative"
                >
                  {item.icon}
                  {item.name}
                  {(item as any).unread > 0 && (
                    <span className="ml-1 min-w-[16px] h-[16px] px-1 bg-rose-500 text-white rounded-full flex items-center justify-center text-[9px] font-bold">
        ...
        <div className="px-4 md:px-8 py-6">
          <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="rounded-xl border border-amber-500/20 bg-gradient-to-br from-amber-500/20 to-transparent p-3">
                <p className="text-[10px] uppercase tracking-[0.15em] text-amber-200/80 font-medium">Unread Messages</p>
                <div className="mt-2 flex items-center justify-between">
                  <p className="text-2xl font-extrabold text-white">{messengerUnread}</p>
                  <MessageCircle className="text-amber-300" size={16} />
                </div>
              </div>
              <div className="rounded-xl border border-sky-500/20 bg-gradient-to-br from-sky-500/20 to-transparent p-3">
                <p className="text-[10px] uppercase tracking-[0.15em] text-sky-200/80 font-medium">Online Users</p>
                <div className="mt-2 flex items-center justify-between">
                  <p className="text-2xl font-extrabold text-white">{onlineUsers.length}</p>
                  <Users className="text-sky-300" size={16} />
                </div>
              </div>
              <div className="rounded-xl border border-emerald-500/20 bg-gradient-to-br from-emerald-500/20 to-transparent p-3">
                <p className="text-[10px] uppercase tracking-[0.15em] text-emerald-200/80 font-medium">Campus Updates</p>
                <div className="mt-2 flex items-center justify-between">
                  <p className="text-2xl font-extrabold text-white">{updatesUnread}</p>
                  <Bell className="text-emerald-300" size={16} />
                </div>
              </div>
            </div>

            {/* Quick Link to JARVIS */}
            <div className="p-5 rounded-2xl relative overflow-hidden group cursor-pointer border border-indigo-400/30 bg-gradient-to-br from-slate-900 to-indigo-950/70 hover:border-indigo-300/50 transition-all" onClick={() => { setActiveRoom('dm-ai-assistant'); setView('messenger'); }}>
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="px-2 py-0.5 rounded-full bg-indigo-500 text-white text-[8px] font-bold uppercase">AI Assistant</span>
                </div>
                <h3 className="text-base font-bold text-white mb-1 group-hover:text-indigo-400 transition-colors">JARVIS AI</h3>
                <p className="text-gray-400 text-xs leading-relaxed mb-3">
                  Ask me anything about MSU, academics, or general knowledge. Click to start chatting!
                </p>
                <div className="flex items-center gap-2 text-indigo-400 text-xs font-bold">
                  <span className="flex items-center gap-1">Launch <ArrowRight size={12} /></span>
                  <button onClick={(e) => { e.stopPropagation(); setSpeechEnabled(v => !v); }} className="px-2 py-0.5 rounded bg-white/10 text-[9px] text-white">{speechEnabled ? 'Voice On' : 'Voice Off'}</button>
                </div>
              </div>
            </div>
          </div>
  ...
  const renderCampusDetail = () => {
    if (!selectedCampus) return null;

    return (
      <div className="h-full w-full bg-[radial-gradient(circle_at_top,rgba(61,43,22,0.9),#090704_55%)] flex flex-col">
        {/* Header */}
        <div className="border-b border-white/10 bg-black/40 backdrop-blur-sm sticky top-0 z-40">
          <div className="px-4 md:px-8 py-4">
            <div className="max-w-7xl mx-auto flex items-center justify-between">
              ...
              <h1 className="text-2xl font-bold text-white">{selectedCampus.name}</h1>
              <div className="w-20" />
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide">
          <div className="px-4 md:px-8 py-8">
            <div className="max-w-4xl mx-auto space-y-8">
              {/* Campus Hero Section */}
              <div className="rounded-2xl border border-amber-400/30 bg-gradient-to-r from-[#16110a] to-[#1d202f] overflow-hidden p-8">
                <div className="flex flex-col md:flex-row gap-8">
                  <div className="md:w-1/3">
                    <div className="w-full aspect-square rounded-xl overflow-hidden border border-white/20">
                      <CampusLogo slug={selectedCampus.slug} />
                    </div>
                  </div>
                  <div className="md:w-2/3">
                    <h2 className="text-3xl font-bold text-white mb-2">{selectedCampus.name}</h2>
                    <div className="flex items-center gap-2 text-amber-400 mb-4">
                      <MapPin size={16} />
                      <span className="text-sm">{selectedCampus.location}</span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-6">
                      <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                        ...
                        <p className="text-xl font-bold text-amber-400">{selectedCampus.stats.students}</p>
                      </div>
                      <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                        <p className="text-xs text-gray-400 mb-1">Courses</p>
                        <p className="text-xl font-bold text-amber-400">{selectedCampus.stats.courses}</p>
                      </div>
                    </div>
                    <div className="flex gap-3">
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="px-4 py-2 rounded-lg border border-amber-500 text-amber-400 font-bold text-sm hover:bg-amber-500/10 transition-colors"
                      >
                        View on Map
                      </a>
                    </div>
              ...
              <div className="rounded-2xl border border-white/10 bg-black/30 backdrop-blur-sm p-6">
                <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                  <Users size={20} className="text-amber-500" /> Groups at {selectedCampus.name}
                </h3>
                <div className="space-y-3">
                  {groups
                    .filter(g => g.campus === selectedCampus.name)
                    .slice(0, 6)
                    .map(group => (
                      <div key={group.id} className="flex items-center justify-between p-4 rounded-xl bg-white/5 border border-white/10 hover:border-amber-500/30 transition-all">
                        <div>
                        ...
                        <button
                          ...
                          className="px-4 py-2 rounded-lg bg-amber-500 text-black font-bold text-sm hover:bg-amber-400 transition-colors whitespace-nowrap"
                        >
                          Join
                        </button>
                      </div>
                    ))}
                  {groups.filter(g => g.campus === selectedCampus.name).length === 0 && (
                    <p className="text-center text-gray-400 py-6">No groups yet in this campus</p>
                  )}
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-white/10 bg-black/30 backdrop-blur-sm p-6">
                <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                  <Bell size={20} className="text-sky-400" /> Campus Announcements
                </h3>
                <div className="p-6 rounded-xl bg-white/5 border border-white/10 text-center">
                  <p className="text-gray-400 text-sm">
                    Announcements will appear here. Check back for updates!
                  </p>
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-white/10 bg-black/30 backdrop-blur-sm p-6">
                <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                  <Globe size={20} className="text-emerald-400" /> Resources
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {selectedCampus.sources.map((source, i) => (
                    <a
                      key={i}
                      href={source.url}
                      target="_blank"
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <div>
                          <p className="font-semibold text-white group-hover:text-amber-400 transition-colors text-sm">{source.label}</p>
          ...
          <div className="rounded-2xl border border-white/10 bg-black/30 backdrop-blur-sm p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-2 px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-gray-400 hover:text-white hover:bg-white/10 cursor-pointer transition-colors text-sm">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      reader.onload = () => setNewsfeedImagePreview(reader.result as string);
                ...
                <button
                  onClick={async () => {
                    if (!user || !newsfeedText.trim()) return;
                    try {
